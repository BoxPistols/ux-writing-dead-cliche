...
description: "AI特有の日本語クリシェ (羅針盤・心臓部・シナジー・〜にほかなりません等) を検出し、意味を保ったまま論文調の平易な文章に書き直す。日本語の文章・記事・ドキュメントを書くとき、書き直すとき、レビューするときに使う。Triggers: クリシェ, AIっぽい, 平易に, リライト, 推敲, 文章レビュー, 校正, 言い換え, plain writing."
---

# クリシェを握りつぶして平易に書く

対象はAIが日本語に持ち込みがちな定型表現。語彙の問題ではなく情報の欠落として扱う。
...
5. 書き直し後に再チェックし、0件になるまで繰り返す。

## 書き直しの規律 (意味の保存)

- 固有名詞・数値・否定はbeforeとafterで必ず一致させる。落ちていたら書き直しの失敗。
